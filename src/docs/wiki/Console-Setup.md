...
`/cockpit/*` (and `/console/*`) return 404 only when **no** secret is configured (e.g. the
shipped `example` profile, which is never auto-provisioned). The link roster unions the Crew
tab with the active schedule. `racecast links --post` also drops the links into the crew
chat.
...
## Per-person links — issue & revoke

Each person opens **one personal link**. The relay can hand it out two ways, both
generated by `racecast links` (and surfaced in the Control Center's
[Crew Console](Control-Center#crew-console) view, which offers a **Copy funnel link** and
a **Copy internal link** button per commentator next to **Revoke**):
...
Control desk, or Director Panel as role-gated cards.
