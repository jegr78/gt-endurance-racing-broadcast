...
Problem → fix. When in doubt, run `python3 src/scripts/preflight.py` first — it catches
...
| Problem | Fix |
|---------|-----|
| Feed says *"Sign in to confirm you're not a bot"* | Refresh cookies (`python3 src/relay/get-cookies.py chrome`) and make sure **deno** is installed — the feeds need both. |
...
| Problem | Fix |
|---------|-----|
| HUD is blank in OBS | The relay draws the HUD — make sure it's running (`python3 tools/run-relay.py`). |
