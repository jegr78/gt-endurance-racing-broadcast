# GT Endurance Racing Broadcast — Setup Guide

This guide has moved to the project wiki, which is always current:

- **How the system works:**
  <https://github.com/jegr78/gt-endurance-racing-broadcast/wiki/Architecture>
- **Set up the broadcast PC** (step by step):
  <https://github.com/jegr78/gt-endurance-racing-broadcast/wiki/Set-up-the-broadcast-PC>
- **Run an event** (the producer's checklist):
  <https://github.com/jegr78/gt-endurance-racing-broadcast/wiki/Run-an-event>
- **The relay — how the feeds work:**
  <https://github.com/jegr78/gt-endurance-racing-broadcast/wiki/Relay-Mode>
- **If something goes wrong:**
  <https://github.com/jegr78/gt-endurance-racing-broadcast/wiki/If-something-goes-wrong>
- **Who does what** (roles + streamer requirements):
  <https://github.com/jegr78/gt-endurance-racing-broadcast/wiki/Who-does-what>

For the quickstart commands, start with `README_SETUP.md` in this package.
