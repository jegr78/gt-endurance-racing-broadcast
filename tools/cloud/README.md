...
- **Reboot once** after provisioning so the `racecast` **autologin desktop** comes up on
  `:0` (OBS + Discord autostart into it; RustDesk mirrors that display). The GPU driver
  itself loads without a reboot, but the desktop session needs one.
