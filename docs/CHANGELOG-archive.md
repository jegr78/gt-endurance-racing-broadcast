# Changelog (archive: pre-0.1.0 reset)

> This is the historical changelog from before the version baseline was reset to
> `0.1.0`. It covers the original `IRO_Broadcast_Setup` line and the
> `1.x`/`2.0.0` numbering that preceded the rebrand to **GT Endurance Racing
> Broadcast**. After the rebrand and the introduction of multi-league profiles,
> the project was rebaselined to `0.1.0` (pre-1.0, still stabilizing); the old
> tags and releases were removed. Kept here as a development record only — the
> live changelog restarts at `CHANGELOG.md`.
